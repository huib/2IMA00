...
public class Kernelization {
    ...
    private int k; // alg parameter k
    private int ChangeInK = 0; // this integer should be added to the actual parameter k!

    private List<Integer> vertexSet = new ArrayList<String>(graph.vertexSet()); // set to arraylist
    ...
    for ( int i = 0; i < vertexSet.size(); i++ ) {
        ...
        if (DegreeOfVertex == 2) {
            ...
            List<Integer> neighbors = graph.neighborListOf(v); // get neighbors a and b of vertex v (allowing possibly a = b)
        ...
        if (containsEdge("v v")) // Returns true if this graph contains the specified edge.
        {
            removeVertex(v);
            ChangeInK -= 1; // reduce k by 1
        }
    ...
    //k += ChangeInK; // apply changes to k
}
